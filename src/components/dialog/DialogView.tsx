...
export default class DialogView extends Component<any, any> {
	...
	static transform(transform: any, animated: any, animatesOnly = null) {
		DeviceEventEmitter.emit("transformRoot", { transform, animated, animatesOnly });
	}

	static restore(animated: any, animatesOnly = null) {
	...
	getChildContext() {
		...
		if (!registerTopViewHandler) {
			registerTopViewHandler = (handler: any) => {
				this.handlers.push(handler);
			};
			unregisterTopViewHandler = (handler: any) => {
	...
	transform(e: any) {
		...
		transform.map((item: { [x: string]: any; }) => {
...
if (!(AppRegistry as any).registerComponentOld) {
	(AppRegistry as any).registerComponentOld = AppRegistry.registerComponent;
}
...
AppRegistry.registerComponent = function (appKey, componentProvider) {
	...
	return (AppRegistry as any).registerComponentOld(appKey, () => RootElement);
}
